...
freebayes -f /path/to/reference/GRChg38.fa -iXu -C 2 -q 20 -n 3 -E 1 -m 30 --min-coverage 6 --limit-coverage 100000 bam/${ID}_minitagged_sorted.bam > vcf/${ID}.vcf
vartrix --umi --mapq 30 -b bam/${ID}_minitagged_sorted.bam -c ${DIR}/${ID}/outs/barcodes.tsv --scoring-method coverage --threads 16 --ref-matrix cts/${ID}_ref.mtx \
--out-matrix cts/${ID}_alt.mtx -v vcf/${ID}.vcf --fasta /path/to/reference/GRChg38.fa
