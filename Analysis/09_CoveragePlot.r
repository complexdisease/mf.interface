...
mapped <- !is.na(bm$external_gene_name[match(genes$gene_id, bm$entrezgene_id)])
genes <- genes[mapped]
names(genes) <- bm$external_gene_name[match(genes$gene_id, bm$entrezgene_id)][mapped]
...
seu@meta.data$subclass=factor(seu@meta.data$subclass,levels = c('VCT',"EVT","SCT","STM","FB","PV","Epi",
                                                "Endo","B","T","dNK","M","HB","cDC","Erythrocyte"))
g1=c("TENM3",'HLA-G',"CYP19A1",'DKK1','BMP5',"MEOX2",'MECOM','CDH5','BCL11A','CD3D','GNLY','MS4A7',"CD74",'HBB')
Idents(seu)="subclass"
p=list()
for (i in 1:length(g1)){
p[[i]]=CoveragePlot(object = seu,region = PR[g1[i]],sep = c(":", "-"),group.by="subclass",extend.upstream = 0,
